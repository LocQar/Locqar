...
export const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      ...
      style={{
        backgroundColor: isLight ? `${config.color}22` : config.bg,
        color: config.color,
        border: `1px solid ${config.color}${isLight ? '45' : '30'}`,
      }}
    >
      <span
...
export const DeliveryMethodBadge = ({ method }) => {
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: `${config.color}${isLight ? '20' : '12'}`,
        border: `1px solid ${config.color}${isLight ? '40' : '25'}`,
      }}
    >
...
export const RoleBadge = ({ role, customRoles = [] }) => {
  ...
  return (
    <span
      ...
      style={{
        backgroundColor: `${r.color}${isLight ? '22' : '18'}`,
        color: r.color,
        border: `1px solid ${r.color}${isLight ? '45' : '30'}`,
      }}
    >
      <Shield size={10} />
      {r.name}
