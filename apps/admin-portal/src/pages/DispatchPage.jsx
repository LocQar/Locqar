      ...
      <div className="flex items-center gap-1 mb-6 p-1 rounded-xl w-fit" style={{ backgroundColor: theme.bg.tertiary }}>
        {['Outgoing', 'Route Planning', 'Driver Assignment'].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            ...
            style={{ backgroundColor: activeTab === tab ? (theme.name === 'light' ? theme.status.error : theme.accent.primary) : 'transparent', color: activeTab === tab ? '#fff' : theme.text.muted }}>
            {tab}
          </button>
        ))}
      </div>
          ...
          <div className="flex flex-col md:flex-row gap-3 mb-4">
            ...
            <div className="flex gap-1 p-1 rounded-xl" style={{ backgroundColor: theme.bg.tertiary }}>
              {[['all', 'All'], ['ready', 'Ready'], ['assigned', 'Assigned'], ['in_transit', 'In Transit'], ['delivered', 'Delivered']].map(([v, l]) => (
                <button key={v} onClick={() => { setFilter(v); setPage(1); }} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ backgroundColor: filter === v ? (theme.name === 'light' ? theme.status.error : theme.accent.primary) : 'transparent', color: filter === v ? '#fff' : theme.text.muted }}>
                  {l}
                </button>
              ))}
            </div>
            ...
            <>
              ...
              <div className="flex gap-1 p-1 rounded-xl mb-5 w-fit" style={{ backgroundColor: theme.bg.tertiary }}>
                {['stops', 'packages', 'timeline'].map(t => (
                  <button key={t} onClick={() => setRouteTab(t)} className="px-4 py-1.5 rounded-lg text-sm capitalize font-medium" style={{ backgroundColor: routeTab === t ? (theme.name === 'light' ? theme.status.error : theme.accent.primary) : 'transparent', color: routeTab === t ? '#fff' : theme.text.muted }}>
                    {t}
                  </button>
                ))}
              </div>
