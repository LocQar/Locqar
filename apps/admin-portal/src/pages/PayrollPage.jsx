...
import {
  ...
  CheckCircle2, AlertTriangle, CalendarDays, ChevronDown,
  UserCheck, Package, Truck, Download,
} from 'lucide-react';
...
function calcDeductions(monthlyGross) {
  ...
  return { ssnitEmployee, ssnitEmployer, incomeTax, totalDeductions, netPay };
}
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between p-5" style={{ borderBottom: `1px solid ${theme.border.primary}` }}>
          <div>
            <p style={{ color: theme.text.muted, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600 }}>Payroll</p>
            <h2 style={{ color: theme.text.primary, fontWeight: 700, fontSize: 16, marginTop: 2 }}>
              Payslip — {period?.label ?? ''}
            </h2>
          </div>
          <button onClick={onClose} style={{ color: theme.text.muted, padding: 6, borderRadius: 8 }}>
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="p-4 rounded-2xl" style={{ background: theme.bg.input }}>
            ...
            {record.deliveryEarnings > 0 && row(
              `Delivery Pay (${record.deliveryCount} × ${record.employeeType === 'courier' ? 'GH₵12' : 'GH₵8'})`,
              fmt(record.deliveryEarnings)
            )}
            {record.bonus > 0 && row('Performance Bonus', fmt(record.bonus))}
            {divider()}
            {row('Gross Pay', fmt(record.grossPay), false, false)}
          </div>
...
export const PayrollPage = ({ activeSubMenu, loading, addToast }) => {
  ...
  const [confirmDialog, setConfirmDialog] = useState(null); // { type: 'approve'|'pay', periodId }
  const [activeTab, setActiveTab] = useState(activeSubMenu || 'Overview');

  // Sync activeTab when sidebar sub-menu changes
  React.useEffect(() => {
    if (activeSubMenu) setActiveTab(activeSubMenu);
  }, [activeSubMenu]);
  ...
  const currentPeriodRecords = useMemo(() => {
    ...
    const staffRecords   = (staffData ?? []).map(s => buildLiveRecord(s, 'staff'));
    const courierRecords = (couriersData ?? []).map(c => buildLiveRecord(c, 'courier', LIVE_COURIER_DELIVERIES[c.id] ?? 0));
    const driverRecords  = (driversData ?? []).map(d => buildLiveRecord(d, 'driver', LIVE_DRIVER_DELIVERIES[d.id] ?? 0));
    return [...staffRecords, ...courierRecords, ...driverRecords];
  }, [selectedPeriodId]);

  // ── Filtered/sorted records for Payslips tab ──
  const displayedRecords = useMemo(() => {
    let list = currentPeriodRecords;
    ...
    return list;
  }, [currentPeriodRecords, typeFilter, search]);

  // ── Metric card values (always from current/most-recent period) ──
  const currentPeriod = periods.find(p => p.id === 'PP-2026-03') ?? periods[0];
  const liveTotalNet = useMemo(() => currentPeriodRecords.reduce((s, r) => s + r.netPay, 0), [currentPeriodRecords]);
  const liveSsnit = useMemo(() => currentPeriodRecords.reduce((s, r) => s + r.ssnitEmployer, 0), [currentPeriodRecords]);
  const draftCount = periods.filter(p => p.status === 'draft').length;
  const paidCount = currentPeriodRecords.filter(r => r.netPay > 0).length;
  const totalCount = currentPeriodRecords.length;

  // ── Approve / Pay handlers ──
  const handleApprove = (periodId) => {
    setPeriods(prev => prev.map(p =>
      p.id === periodId ? { ...p, status: 'approved', approvedBy: 'John Doe', approvedAt: new Date().toISOString().split('T')[0] } : p
    ));
    setConfirmDialog(null);
    addToast({ type: 'success', message: 'Payroll period approved successfully' });
  };
  ...
  const StatusBadge = ({ status }) => {
    ...
    const Icon = status === 'paid' ? CheckCircle2 : status === 'approved' ? CheckCircle2 : AlertTriangle;
    return (
  ...
  const card = (bg, border) => ({ background: bg, border: `1.5px solid ${border}`, borderRadius: 16, padding: '16px 20px' });

  // ============ OVERVIEW TAB ============
  const renderOverview = () => (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 style={{ color: theme.text.primary, fontWeight: 600, fontSize: 15 }}>Pay Periods</h2>
      </div>
    ...
    <div>
      ...
      <div className="rounded-2xl overflow-hidden border" style={{ borderColor: theme.border.primary }}>
        <table className="w-full text-sm">
          <thead>
            <tr style={{ background: theme.bg.input }}>
              {['Employee', 'Type', 'Role', 'Deliveries', 'Gross', 'SSNIT (EE)', 'Income Tax', 'Net Pay', ''].map(h => (
                <th key={h} className="text-left px-4 py-3" style={{ color: theme.text.muted, fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {displayedRecords.length === 0 ? (
              <tr>
                <td colSpan={9} className="text-center py-12" style={{ color: theme.text.muted }}>No employees match your filters</td>
              </tr>
            ) : displayedRecords.map((r, i) => (
              <tr key={`${r.employeeType}-${r.employeeId}-${i}`} style={{ background: i % 2 === 0 ? 'transparent' : theme.bg.hover, borderTop: `1px solid ${theme.border.primary}` }}>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
  ...
  const renderPayPeriods = () => (
    <div className="space-y-6">
      ...
      {/* Historical table */}
      <div>
        <h3 style={{ color: theme.text.primary, fontWeight: 600, marginBottom: 12, fontSize: 14 }}>All Pay Periods</h3>
        <div className="rounded-2xl overflow-hidden border" style={{ borderColor: theme.border.primary }}>
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: theme.bg.input }}>
                {['Period', 'Date Range', 'Employees', 'Gross', 'Deductions', 'Net', 'Status', 'Approved By', 'Paid On'].map(h => (
                  <th key={h} className="text-left px-4 py-3" style={{ color: theme.text.muted, fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</th>
                ))}
              </tr>
            </thead>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.4)' }}>
          <div className="rounded-2xl p-6 w-80 shadow-2xl" style={{ background: theme.bg.card, border: `1px solid ${theme.border.primary}` }}>
            <div className="w-11 h-11 rounded-full flex items-center justify-center mx-auto mb-3" style={{ background: isApprove ? '#3B82F615' : '#10B98115' }}>
              <CheckCircle2 size={22} style={{ color: isApprove ? '#3B82F6' : '#10B981' }} />
            </div>
            <h3 style={{ color: theme.text.primary, fontWeight: 700, textAlign: 'center', fontSize: 16, marginBottom: 6 }}>
            ...
            <p style={{ color: theme.text.muted, textAlign: 'center', fontSize: 13, marginBottom: 20 }}>
              {isApprove
                ...
                : `Confirm that ${confirmDialog.label} payroll has been disbursed?`}
            </p>
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <MetricCard
          ...
          value={draftCount === 0 ? 'None' : `${draftCount} period${draftCount > 1 ? 's' : ''}`}
          icon={Clock}
          loading={loading}
        />
        <MetricCard
