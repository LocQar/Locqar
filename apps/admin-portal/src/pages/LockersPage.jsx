        ...
        <>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1 p-1 rounded-xl" style={{ backgroundColor: theme.bg.tertiary }}>
              <button onClick={() => setViewMode('grid')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium" style={{ backgroundColor: viewMode === 'grid' ? (theme.name === 'light' ? theme.status.error : theme.accent.primary) : 'transparent', color: viewMode === 'grid' ? '#fff' : theme.text.muted }}>
                <LayoutGrid size={13} /> Grid
              </button>
              <button onClick={() => setViewMode('list')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium" style={{ backgroundColor: viewMode === 'list' ? (theme.name === 'light' ? theme.status.error : theme.accent.primary) : 'transparent', color: viewMode === 'list' ? '#fff' : theme.text.muted }}>
                <List size={13} /> List
              </button>
              <button onClick={() => setViewMode('terminal')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium" style={{ backgroundColor: viewMode === 'terminal' ? (theme.name === 'light' ? theme.status.error : theme.accent.primary) : 'transparent', color: viewMode === 'terminal' ? '#fff' : theme.text.muted }}>
            ...
            <div className="flex flex-col md:flex-row gap-3">
              ...
              <div className="flex gap-1 p-1 rounded-xl" style={{ backgroundColor: theme.bg.tertiary }}>
                {[['all', 'All'], ['available', 'Available'], ['occupied', 'Occupied'], ['reserved', 'Reserved'], ['maintenance', 'Maint.']].map(([v, l]) => (
                  <button key={v} onClick={() => setStatusFilter(v)} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ backgroundColor: statusFilter === v ? (theme.name === 'light' ? theme.status.error : theme.accent.primary) : 'transparent', color: statusFilter === v ? '#fff' : theme.text.muted }}>{l}</button>
                ))}
              </div>
            </div>
            <div className="flex flex-wrap gap-3">
              <div className="flex gap-1 p-1 rounded-xl" style={{ backgroundColor: theme.bg.tertiary }}>
                {[['all', 'All Terminals'], ...terminalsData.map(t => [t.name, t.name.split(' ')[0]])].map(([v, l]) => (
                  <button key={v} onClick={() => setTerminalFilter(v)} className="px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap" style={{ backgroundColor: terminalFilter === v ? (theme.name === 'light' ? theme.status.error : theme.accent.primary) : 'transparent', color: terminalFilter === v ? '#fff' : theme.text.muted }}>{l}</button>
                ))}
              </div>
              <div className="flex gap-1 p-1 rounded-xl" style={{ backgroundColor: theme.bg.tertiary }}>
                {[['all', 'All Sizes'], ...SIZE_LABELS.map(s => [s, s])].map(([v, l]) => (
                  <button key={v} onClick={() => setSizeFilter(v)} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ backgroundColor: sizeFilter === v ? (theme.name === 'light' ? theme.status.error : theme.accent.primary) : 'transparent', color: sizeFilter === v ? '#fff' : theme.text.muted }}>{l}</button>
                ))}
              </div>
            </div>
          </div>
                ...
                <thead>
                  <tr style={{ borderBottom: `1px solid ${theme.border.primary}` }}>
                    {[['id', 'ID'], ['doorNo', 'Door #', 'hidden sm:table-cell'], ['terminal', 'Terminal'], ['sizeLabel', 'Size', 'hidden md:table-cell'], ['status', 'Status'], ['doorState', 'Door', 'hidden lg:table-cell'], ['package', 'Package', 'hidden lg:table-cell'], ['temp', 'Temp', 'hidden md:table-cell'], ['battery', 'Battery', 'hidden md:table-cell']].map(([field, label, hide]) => (
                      <th key={field} onClick={() => sortFn(field)} className={`text-left p-3 text-xs font-semibold uppercase cursor-pointer select-none ${hide || ''}`} style={{ color: sort.field === field ? (theme.name === 'light' ? theme.status.error : theme.accent.primary) : theme.text.muted }}>
