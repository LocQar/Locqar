...
const Sidebar = ({ isCollapsed, setIsCollapsed, activeMenu, setActiveMenu, activeSubMenu, setActiveSubMenu, theme, userRole, isMobile, onCloseMobile, customRoles = [] }) => {
  ...
  return (
    <>
      {isMobile && <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" onClick={onCloseMobile} />}
      <aside className={`${isMobile ? 'fixed inset-y-0 left-0 z-50' : ''} ${isCollapsed && !isMobile ? 'w-20' : 'w-72'} border-r flex flex-col transition-all duration-300`} style={{ backgroundColor: theme.bg.secondary, borderColor: theme.border.primary }}>
        {/* Logo */}
        <div className="h-16 flex items-center justify-between px-4 border-b" style={{ borderColor: theme.border.primary }}>
          {(!isCollapsed || isMobile) && (
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ background: `linear-gradient(135deg, ${theme.accent.primary}, ${theme.accent.secondary})` }}>
                <span className="text-white font-bold text-xs">LQ</span>
              </div>
              <span className="font-bold text-base tracking-tight" style={{ color: theme.text.primary }}>LocQar</span>
            </div>
          )}
          {isCollapsed && !isMobile && (
            <div className="w-7 h-7 rounded-lg flex items-center justify-center mx-auto" style={{ background: `linear-gradient(135deg, ${theme.accent.primary}, ${theme.accent.secondary})` }}>
              <span className="text-white font-bold text-xs">LQ</span>
            </div>
          )}
          {!isMobile && (
            <button onClick={() => setIsCollapsed(!isCollapsed)} className="p-1.5 rounded-lg transition-colors" style={{ backgroundColor: 'transparent', color: theme.icon.muted }}
              onMouseEnter={e => { e.currentTarget.style.backgroundColor = theme.bg.hover; e.currentTarget.style.color = theme.icon.primary; }}
              onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = theme.icon.muted; }}
            >
              {isCollapsed ? <ChevronRight size={16} /> : <ChevronLeft size={16} />}
            </button>
          )}
          {isMobile && <button onClick={onCloseMobile} className="p-1.5 rounded-lg" style={{ color: theme.icon.muted }}><X size={16} /></button>}
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2.5">
          {menuGroups.map((group, idx) => (
            <div key={group.label} className={idx > 0 ? 'mt-5' : ''}>
              {(!isCollapsed || isMobile) && (
                <p className="px-2 mb-1.5 text-[10px] font-bold uppercase tracking-widest" style={{ color: theme.text.muted }}>
                  {group.label}
                </p>
              )}
                  ...
                  return (
                    <div key={item.id}>
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-2.5 py-2 rounded-xl text-sm transition-all duration-150 relative"
                        style={{
                          backgroundColor: isActive ? theme.accent.light : 'transparent',
                          color: isActive ? theme.accent.primary : theme.text.secondary,
                        }}
                        onMouseEnter={e => { if (!isActive) { e.currentTarget.style.backgroundColor = theme.bg.hover; e.currentTarget.style.color = theme.text.primary; } }}
                        onMouseLeave={e => { if (!isActive) { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = theme.text.secondary; } }}
                      >
                        {/* Active left-border indicator */}
                        {isActive && (
                          <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-r-full" style={{ backgroundColor: theme.accent.primary }} />
                        )}
                        <item.icon size={18} style={{ color: isActive ? theme.accent.primary : theme.icon.primary }} />
                        {(!isCollapsed || isMobile) && (
                          <>
                            ...
                            {item.subItems && (
                              <ChevronDown
                                size={15}
                                style={{ color: isActive ? theme.accent.primary : theme.icon.muted }}
                                className={`transition-transform duration-200 ${expandedMenus.includes(item.id) ? 'rotate-180' : ''}`}
                              />
                            )}
                          </>
                        )}
                      ...
                      {(!isCollapsed || isMobile) && item.subItems && expandedMenus.includes(item.id) && (
                        <div className="mt-0.5 ml-3.5 pl-4 space-y-0.5 py-1" style={{ borderLeft: `1px solid ${theme.border.primary}` }}>
                          {item.subItems.map(sub => {
                            ...
                            return (
                              <button
                                ...
                                style={{
                                  color: isSubActive ? theme.accent.primary : theme.text.muted,
                                  backgroundColor: isSubActive ? theme.accent.light : 'transparent',
                                }}
                                onMouseEnter={e => { if (!isSubActive) { e.currentTarget.style.color = theme.text.secondary; e.currentTarget.style.backgroundColor = theme.bg.hover; } }}
                                onMouseLeave={e => { if (!isSubActive) { e.currentTarget.style.color = theme.text.muted; e.currentTarget.style.backgroundColor = 'transparent'; } }}
                              >
                                {sub}
                              </button>
                            );
        ...
        <div className="p-2.5 border-t" style={{ borderColor: theme.border.primary }}>
          {/* Settings button */}
          <button
            onClick={() => { setActiveMenu('settings'); if (isMobile) onCloseMobile(); }}
            className="w-full flex items-center gap-3 px-2.5 py-2 rounded-xl text-sm font-medium transition-all duration-150 mb-2"
            style={{
            ...
            onMouseEnter={e => { if (activeMenu !== 'settings') { e.currentTarget.style.backgroundColor = theme.bg.hover; e.currentTarget.style.color = theme.text.primary; } }}
            onMouseLeave={e => { if (activeMenu !== 'settings') { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = theme.text.secondary; } }}
          >
            <Settings size={18} style={{ color: activeMenu === 'settings' ? theme.accent.primary : theme.icon.primary }} />
          ...
          >
            ...
            <div className="relative shrink-0">
              ...
              <span
                ...
                style={{ backgroundColor: theme.status.success, borderColor: theme.bg.tertiary }}
              />
            </div>

            {(!isCollapsed || isMobile) && (
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold leading-none truncate" style={{ color: theme.text.primary }}>
                  Admin
                </p>
                <p className="text-[10px] mt-0.5 truncate" style={{ color: theme.text.muted }}>
